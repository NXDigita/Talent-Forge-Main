...
export default function Home() {
  return (
    <div className="min-h-screen bg-background">
      {/* Hero Section */}
      <section className="pt-32 pb-20 px-4 relative overflow-hidden bg-[linear-gradient(135deg,#EEF2FF_0%,#DBEAFE_40%,#F7F3EE_100%)]">
        <div className="absolute inset-0 z-0 opacity-25 bg-[radial-gradient(ellipse_at_top,#BFDBFE_0%,transparent_60%)]" />
        <div className="container mx-auto max-w-6xl relative z-10 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50/70 border border-zinc-200 text-blue-700 text-sm font-medium mb-6">
            <Zap size={14} />
      ...
      <section className="py-24 px-4 relative">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { title: "Computer Science", icon: <Code2 className="w-8 h-8 text-blue-700" />, desc: "Full-stack, Backend, Frontend, DevOps" },
              { title: "AI & Data", icon: <Activity className="w-8 h-8 text-amber-500" />, desc: "Machine Learning, Data Engineering, Analytics" },
              { title: "Electronics (ECE)", icon: <Cpu className="w-8 h-8 text-blue-600" />, desc: "Embedded Systems, IoT, VLSI, Robotics" },
              ...
              { title: "Business/Management", icon: <ShieldCheck className="w-8 h-8 text-blue-500" />, desc: "Product Management, Technical Sales, Ops" }
            ].map((domain, i) => (
      ...
      {/* CTA Banner */}
      <section className="py-24 px-4 bg-gradient-to-r from-violet-50 to-amber-50 border-t border-blue-200">
